...
int main() {
    ...
    t1=urdtsc();
    encrypted_length = RSA_private_encrypt(strlen((char *)plaintext), plaintext, encrypted, rsa, RSA_PKCS1_PADDING);
    t2=urdtsc();
    t_rsa = (t2-t1)*5/17;
    printf("rsa time:%ld\n",t_rsa);
    ...
    printf("Encrypted (signed) message:\n");
    ...
    decrypted_length = RSA_public_decrypt(encrypted_length, encrypted, decrypted, rsa, RSA_PKCS1_PADDING);
    if (decrypted_length == -1) {
        handleErrors();
    }

    printf("Decrypted (verified) message: %.*s\n", decrypted_length, decrypted);
