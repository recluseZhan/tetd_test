...
#include <sys/stat.h>
#include <fcntl.h>
#define DEVNAME "/dev/log_dev"
#include <sys/types.h>
#define MEMORY_BUFFER_SIZE 4096
static char memoryBuffer[MEMORY_BUFFER_SIZE];
static size_t currentSize = 0;
void writeToMemoryBuffer(const char *message){
    size_t messageLen = strlen(message);
    if(currentSize + messageLen < MEMORY_BUFFER_SIZE){
        strcpy(memoryBuffer + currentSize, message);
        currentSize += messageLen;
        
        int fd = 0;
        fd = open(DEVNAME,O_RDWR);
        if(fd >= 0){
            printf("fd:%d\n",fd);
            read(fd,memoryBuffer,sizeof(memoryBuffer));
            close(fd);
        }
    }else{
        fprintf(stderr, "memory buff full\n");
    }
}
void printMemoryBuffer(){
    printf("\nxin current:%p, %zu\n\n",(void*)memoryBuffer, sizeof(memoryBuffer));
}
//
