...
//xin
/**
 * rmi_ipa_block() - Block access to an IPA range
 * @rd: PA of the RD
 * @ipa: IPA to block
 *
 * Block access to a specified IPA range in the realm.
 *
 * Return: RMI return code
 */
static inline int rmi_ipa_block(unsigned long rd, unsigned long ipa)
{
	struct arm_smccc_res res;

	arm_smccc_1_1_invoke(SMC_RMI_IPA_BLOCK, rd, ipa, &res);

	return res.a0;
}

/**
 * rmi_ipa_unblock() - Unblock access to an IPA range
 * @rd: PA of the RD
 * @ipa: IPA to unblock
 *
 * Restore access to a previously blocked IPA range in the realm.
 *
 * Return: RMI return code
 */
static inline int rmi_ipa_unblock(unsigned long rd, unsigned long ipa)
{
	struct arm_smccc_res res;

	arm_smccc_1_1_invoke(SMC_RMI_IPA_UNBLOCK, rd, ipa, &res);
