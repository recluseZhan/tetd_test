...
//xin
unsigned long smc_ipa_block(unsigned long rd_addr, unsigned long map_addr)
{
	struct granule *g_rd;
	struct rd *rd;
	struct granule *g_table_root;
	...
	if (s2tte_is_assigned_ram(s2tte, RTT_PAGE_LEVEL)) {
		/* Clear access permissions (AP=0b00) and set XN bit */
		s2tte &= ~S2TTE_AP_MASK;
		s2tte |= S2TTE_XN;
		s2tte_write(&s2tt[wi.index], s2tte);
		invalidate_page(&s2_ctx, map_addr);
...
unsigned long smc_ipa_unblock(unsigned long rd_addr, unsigned long map_addr)
{
	struct granule *g_rd;
	struct rd *rd;
	struct granule *g_table_root;
	...
	if (s2tte_is_assigned_ram(s2tte, RTT_PAGE_LEVEL)) {
		/* Set RW access (AP=0b11) and clear XN bit */
		s2tte |= S2TTE_AP_MASK;
		s2tte &= ~S2TTE_XN;
		s2tte_write(&s2tt[wi.index], s2tte);
		invalidate_page(&s2_ctx, map_addr);
