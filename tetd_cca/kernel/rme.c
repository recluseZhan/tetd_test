...
void realm_destroy_undelegate_range(struct realm *realm,
					   unsigned long ipa,
					   ssize_t size)
{
...
//xin
EXPORT_SYMBOL(realm_destroy_undelegate_range);
//
...
//xin
EXPORT_SYMBOL(kvm_realm_unmap_range);
//
			...
			struct kvm_mmu_memory_cache *memcache)
{
	...
	if(count>=0){
	    count = count - 1;
	    printk("realm=0x%lx,hva=0x%lx,base_ipa=0x%lx,dst_page=0x%lx,map_size=0x%lx,cache=0x%lx\n",realm,hva,base_ipa,dst_page,map_size,memcache);
	}
	*/
	//
...
//xin
EXPORT_SYMBOL(realm_map_protected);
//
